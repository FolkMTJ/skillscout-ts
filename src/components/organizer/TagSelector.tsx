...
export default function TagSelector({ selectedTags, onChange, maxTags = 10 }: TagSelectorProps) {
  ...
  useEffect(() => {
    fetch('/api/tags')
      .then(res => res.json())
      .then(data => {
        if (data.tags) setTagsData(data.tags);
        setLoading(false);
      })
      .catch(err => {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Categories */}
      {loading ? (
        <div className="text-center py-4 text-sm text-foreground-400">กำลังโหลด Tags...</div>
      ) : (
        <div className="space-y-2">
          {Object.entries(TAG_CATEGORIES).map(([categoryKey, categoryName]) => {
            const tags = getTagsByCategory(categoryKey);
            if (tags.length === 0) return null; // Hide empty categories
