...
import { FiDownload, FiShare2, FiEdit2, FiCheck, FiX, FiAward } from 'react-icons/fi';
...
export default function ShareResultModal({
    ...
    useEffect(() => {
        ...
        const measure = () => {
            ...
            if (isMobile) {
                // Mobile: constrain by container width and 32% of viewport height
                ...
                // Desktop: height-first — fill 92dvh minus header/padding (~48px)
    ...
    const handleShare = async () => {
        try {
            ...
            if (navigator.share && navigator.canShare({ files: [file] })) {
                await navigator.share({ title, text: 'ดูผลลัพธ์ Path Finder ของฉันจาก SkillScout!', files: [file] });
                ...
                {() => (<>
                                    ...
                                    return (
                                        <div style={{ width: `${Math.round(1080 * zoomScale)}px`, height: `${Math.round(1920 * zoomScale)}px`, position: 'relative', flexShrink: 0 }}>
                                            <div style={{ position: 'absolute', top: 0, left: 0, width: '1080px', transformOrigin: 'top left', transform: `scale(${zoomScale})` }}>
                                                <ShareResultCard result={result} userName={effectiveName} />
                                            </div>
                                        </div>
                                    );
                                })()}
                    ...
                    <div className="flex flex-col md:flex-row max-h-[92dvh]">
                        ...
                        <div style={{ position: 'fixed', top: 0, left: '-1100px', width: '1080px', height: '1920px', opacity: 0, pointerEvents: 'none', zIndex: -1 }}>
                            <ShareResultCard ref={captureRef} result={result} userName={effectiveName} />
                        </div>
                            ...
                            <div className="flex-1 flex flex-col px-4 py-4 md:px-8 md:py-8 gap-4 md:gap-5">
                                ...
                                <div className="border border-gray-100 rounded-2xl px-4 py-3 md:px-5 md:py-4 bg-gray-50">
                                    <div className="flex items-center gap-2 mb-3">
                                        <FiAward className="text-[#F2B33D]" size={16} />
                                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">บุคลิกภาพเด่นของคุณ</p>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        {result.topRIASECCodes.slice(0, 3).map((code) => {
                                            const names: Record<string, string> = { R: 'นักปฏิบัติ', I: 'นักคิดวิเคราะห์', A: 'นักสร้างสรรค์', S: 'นักช่วยเหลือ', E: 'นักบริหาร', C: 'นักจัดการ' };
                                            return (
                                                <span key={code} className="inline-flex items-center gap-1.5 bg-[#F2B33D]/10 border border-[#F2B33D]/30 text-[#b8861e] font-bold text-sm px-3 py-1.5 rounded-xl">
                                                    <span className="w-5 h-5 bg-[#F2B33D] text-white rounded-full flex items-center justify-center text-xs font-black">{code}</span>
                                                    {names[code] ?? code}
                                                </span>
                                            );
                                        })}
                                    ...
                                    {result.recommendedCareerDetails && result.recommendedCareerDetails.length > 0 && (
                                        <p className="text-xs text-gray-400 mt-3 leading-relaxed">
                                            อาชีพแนะนำ: <span className="text-gray-600 font-medium">{result.recommendedCareerDetails.slice(0, 2).map(c => c.nameTh).join(', ')}</span>
                                        </p>
                                    )}
                                </div>

                                {/* Desktop Actions (mt-auto pushes to bottom) */}
